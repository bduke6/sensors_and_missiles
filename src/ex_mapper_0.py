...
from ex_dynamic_label_avoidance import DynamicLabelAvoidance
...
for i in range(num_squares):
    x_proj, y_proj = m(x[i], y[i])
    dla.add_entity(x_proj, y_proj, labels[i])
...
def update_positions(current_time):
    if current_time not in history_dict:
        ...
        for j in range(num_squares):
            new_x = squares[j].get_x() + vx[j] * 1e6 * current_time / 100  # Apply time factor for movement
            ...
            # Update DLA for this square's label and leader line
            label_x, label_y = dla.move_square(j, new_x, new_y)
            texts[j].set_position((label_x, label_y))
            
            # Update leader line position (straight line for now)
            lines[j].set_data([new_x + 1500, label_x], [new_y + 3000, label_y])

        # Save positions for this time step
        history_dict[current_time] = positions
    else:
        # If already in history, revert to saved positions
        for j in range(num_squares):

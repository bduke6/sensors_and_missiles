...
for entity_id in np.unique(entity_ids):
    ...
    render_info = entity_config['entities'].get(str(entity_id), {}).get('render', None)
    if render_info:
        # Initial plot for each entity with the specified symbol
        lat, lon = latitudes[entity_ids == entity_id][0], longitudes[entity_ids == entity_id][0]
        x, y = m(lon, lat)
        symbol = render_info.get('symbol', 'o')  # Default to 'o' if no symbol is defined
        color = render_info.get('color', 'blue')  # Default to blue
        size = render_info.get('size', 10)  # Default size
        
        # Use plt.text to plot the symbol as text on the map
        entity_plots[entity_id] = plt.text(x, y, symbol, fontsize=size, color=color, ha='center', va='center', visible=False)
    ...
    for entity_id, plot in entity_plots.items():
        ...
        if entity_mask.any():
            ...
            if (map_config['llcrnrlon'] <= lon <= map_config['urcrnrlon'] and
                ...
                plot.set_position((x, y))  # Update position

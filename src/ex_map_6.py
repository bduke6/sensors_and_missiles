...
entity_plots = {}
for entity_id in entity_config['entities'].keys():
    # Get rendering details from the entity configuration
    render_info = entity_config['entities'][entity_id].get('render', None)
    
    if render_info:
        # Get initial lat/lon for rendering entities without output data
        lat = entity_config['entities'][entity_id].get('lat', None)
        lon = entity_config['entities'][entity_id].get('lon', None)
